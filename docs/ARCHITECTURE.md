...
Events are pushed through the existing `window.dataLayer` interface when available, without installing pixels or analytics scripts. `embed_ready` and `embed_error` fire only from an explicit same-repository callback or a validated iframe message. Browser load heuristics and clicks never imply form readiness, failure or completion. The form team owns step, answer, validation and completion/submission analytics. No name, contact detail, date of birth, health information, answer value or arbitrary URL parameter enters the data layer.
